...
import { ApiError, addProjectMember, advanceProjectStage, beginLogin, createProject, createProjectDeliverable, createProjectIssue, createProjectTask, decideGate, decideGateCheck, decideProjectIssue, getCurrentSession, getProjects, getProjectMembers, getProjectTasks, getProjectActivity, getProjectDeliverables, getProjectGates, getProjectIssues, getProjectResources, getRuntimeConfigStatus, logout, removeProjectMember, replaceProjectMemberRoles, reviewProjectDeliverable, submitGate, updateProject, updateProjectTask, type ActivityPage, type CurrentSession, type Deliverable, type Project, type ProjectGate, type ProjectMember, type ProjectResource, type ProjectTask, type OpenIssue, type RuntimeConfigStatus } from './api'
...
const deliverableTitle = ref(''); const deliverablePhase = ref(''); const deliverableVersion = ref('v1.0'); const deliverableSourceRef = ref(''); const deliverableTaskId = ref(''); const deliverableSaving = ref(false)
...
const issueTitle = ref(''); const issueDescription = ref(''); const issueImpact = ref(''); const issueDecisionRole = ref('OWNER'); const issueInitialStatus = ref<'OPEN' | 'HUMAN_DECISION_REQUIRED'>('OPEN'); const issueSaving = ref(false); const issueDecision = ref(''); const issueOutcome = ref<'OPEN' | 'HUMAN_DECISION_REQUIRED' | 'DECIDED' | 'TRACKING' | 'CLOSED'>('DECIDED'); const issueDeciding = ref(false)
...
async function submitDeliverable() {
  ...
  try { const taskId = deliverableTaskId.value.trim() ? Number(deliverableTaskId.value.trim()) : undefined; if (taskId !== undefined && (!Number.isInteger(taskId) || taskId < 1)) throw new Error('关联任务ID必须是正整数。'); const item = await createProjectDeliverable({ projectId: workspaceProject.value.id, title: deliverableTitle.value.trim(), phase: deliverablePhase.value.trim(), version: deliverableVersion.value.trim() || 'v1.0', sourceRef: deliverableSourceRef.value.trim(), ...(taskId === undefined ? {} : { taskId }), csrfToken: session.value.csrfToken }); workspaceDeliverables.value = [item, ...workspaceDeliverables.value]; deliverableTitle.value = ''; deliverableSourceRef.value = ''; deliverableTaskId.value = '' }
...
/** 由编排者选择同项目决策责任人和任务范围，将Gate送入独立Reviewer评审。 */
async function submitGateForReview(gate: ProjectGate) {
  if (!session.value || gateWorking.value || !['PENDING', 'RETURNED'].includes(gate.status)) return
  gateWorking.value = true; workspaceError.value = ''
  try {
    const [memberList, taskPage] = await Promise.all([getProjectMembers(gate.projectId), getProjectTasks(gate.projectId)])
    const decisionOwnerIndex = Number(prompt(`选择决策责任人编号：\n${memberList.map((m, index) => `${index + 1}. ${m.displayName} (${m.roles.join(',')})`).join('\n')}`, '1')) - 1
    const decisionOwner = memberList[decisionOwnerIndex]
    if (!decisionOwner) throw new Error('未选择有效的决策责任人。')
    const taskIds = taskPage.items.map(task => task.id)
    const selected = prompt(`输入Gate范围任务ID（逗号分隔，默认全部）：\n${taskPage.items.map(task => `${task.id}. ${task.title}`).join('\n')}`, taskIds.join(','))
    const parsedTaskIds = (selected || '').split(',').map(value => Number(value.trim())).filter(Number.isInteger)
    if (!parsedTaskIds.length) throw new Error('Gate至少需要一个任务范围。')
    await submitGate({ gateId: gate.id, decisionOwnerRef: decisionOwner.principalRef, taskIds: parsedTaskIds, deliverableIds: [], csrfToken: session.value.csrfToken })
    workspaceGates.value = await getProjectGates(gate.projectId)
  } catch (error) { workspaceError.value = error instanceof Error ? error.message : 'Gate提交失败，请稍后重试。' }
  finally { gateWorking.value = false }
}
