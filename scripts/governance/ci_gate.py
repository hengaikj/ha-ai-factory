...
def run_gate(payload: dict) -> dict:
    ...
    result = evaluate_gate(payload.get("work", {}), payload.get("context", {}), payload["action"])
    result["enforcement_mode"] = enforcement["mode"]
    result["enforcement_effective"] = result["decision"] == "ALLOW" or (enforcement["mode"] == "advisory" and result["decision"] == "REVIEW_REQUIRED")
    return result
