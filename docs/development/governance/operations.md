...
命令只读取 JSON 输入并输出 JSON 结果。未知任务、缺少基线、缺少人工批准、禁止动作、非零退出码均失败关闭。退出码 2 表示校验或门禁未通过。

## 可信边界

`.agent/governance/*.json` 当前为 `PROPOSED`，不是批准记录。候选分支不能通过修改这些文件为自己授予权限；正式合入仍需独立 Reviewer 和可信 CI。当前没有配置分支保护，不能声称 CI 门禁已生效。
