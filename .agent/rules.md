...
负责人于 2026-09-25 明确要求仅使用 GitHub 账号 `hengaikj` 完成当前项目工作。当前仓库采用单账号自审模式：该账号可完成实现、测试、证据整理、PR 操作和合并准备，不再等待第二个 GitHub 账号。

- 单账号自审不等同于独立 Reviewer 审查；证据必须明确标注执行者和审查者均为 `hengaikj`。
- 分支保护不要求额外批准人数，但必须保留 CI 检查、会话解决、禁止强推和禁止删除等保护。
- 单账号模式不得解除 Contract、Runtime 失败关闭、数据库迁移或部署授权边界；真实副作用仍需对应范围的批准。
- 目标状态可标记为“单账号实现与验证完成”，不得标记为“独立复审通过”。
